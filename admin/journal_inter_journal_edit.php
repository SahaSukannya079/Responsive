...
if (empty($_SESSION['email'])) {
  echo "<script>window.location.assign('login.php');</script>";
    ...
    if (
        ...
        && !empty($_POST["pub"]) && !empty($_POST["pos"])
    ) {
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
         ...
         $stmt = "UPDATE journal_int_journal SET title='$title', official_publication='$pub',
          position = '$pos' WHERE journal_int_journal_id='$val'";
           
         
         $conn->exec($stmt);
         
         echo "<script>window.alert('Status Changed to Available');</script>";
         echo "<script>window.location.assign('journal_inter_journal_action.php');</script>";
        ...
        <div class="row">
            ...
            <main role="main" class="ml-sm-12 col-12">
                <div class="tm-section-wrap">
                    <h2 class="heading">Member of Journal Editorial Boards</h2>
                    <h3 class="head2">Edit To International Journal</h3>
                    <br>
                    <br>
                    <?php
                       require_once "pdo.php";
                       $stmt = $pdo->prepare("SELECT * FROM journal_int_journal where journal_int_journal_id = :val");
                       $stmt->execute(array(":val" => $_GET['value']));
                       $row = $stmt->fetch(PDO::FETCH_ASSOC);
                         $t = htmlentities($row['title']);
                         $o = htmlentities($row['official_publication']);
                         $p = htmlentities($row['position']);
                         $val = $row['journal_int_journal_id']; 
                    ...
                    <div class="container-insert">
                        <form action="" method="POST" class="form">
                            <div class="row">
                                ...
                                <div class="col-65">
                                    <input type="text" name="pub" value="<?= $o ?> ">
                                </div>
                                <br>
                                <br>
                                <div class="col-25">
                                ...
                                <div class="col-65">
                                    <input type="text" name="pos" value="<?= $p ?>">
                                </div>
                                <input type="hidden" name="i_id" value="<?= $val ?>">
                                <br>
                                <div class="sub_btn">
                                <input type="submit" name="submit" value="Edit">
                                <a href="journal_inter_journal_action.php" class="cancelbtn"
