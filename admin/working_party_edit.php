...
if (empty($_SESSION['email'])) {
  echo "<script>window.location.assign('login.php');</script>";
    ...
    if (
        ...
        && !empty($_POST["mship"]) && !empty($_POST["pname"])
    ) {
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
         ...
         $stmt = "UPDATE working_party SET year='$year', membership='$mship',
          party_name= '$pname' WHERE working_party_id='$val'";
           
         
         $conn->exec($stmt);
         
         echo "<script>window.alert('Status Changed to Available');</script>";
         echo "<script>window.location.assign('jworking_party_action.php');</script>";
        ...
        <div class="row">
            ...
            <main role="main" class="ml-sm-12 col-12">
                <div class="tm-section-wrap">
                    <h2 class="heading">Memberships of Working Party / </h2>
                    <h2 class="heading2"> Special Interest Groups </h2>
                    <h3 class="head2"> Edit</h3>
                    <br>
                    <br>
                    <?php
                       require_once "pdo.php";
                       $stmt = $pdo->prepare("SELECT * FROM working_party where working_party_id = :val");
                       $stmt->execute(array(":val" => $_GET['value']));
                       $row = $stmt->fetch(PDO::FETCH_ASSOC);
                         $y = htmlentities($row['year']);
                         $m = htmlentities($row['membership']);
                         $p = htmlentities($row['party_name']);
                         $val = $row['working_party_id']; 
                    ...
                    <div class="container-insert">
                        <form action="" method="POST" class="form">
                            <div class="row">
                                ...
                                <br>
                                <div class="col-25">
                                    <label for="mship"> Membership </label>
                                </div>
                                <div class="col-65">
                                    <input type="text" name="mship" 
                                    value="<?= $m ?>"> 
                                </div>
                                <br>
                                <div class="col-25">
                                ...
                                <div class="col-65">
                                    <input type="text" name="pname" list="pn" 
                                    value="<?= $p ?>">
                                ...
                                <a href="working_party_action.php" class="cancelbtn">Cancel</a>
                                </div>
                            </div>
                        </form>
                    </div>
