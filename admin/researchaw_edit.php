...
if (empty($_SESSION['email'])) {
  echo "<script>window.location.assign('login.php');</script>";
    ...
    if (isset($_POST["submit"]) && !empty($_POST["year"])
        && !empty($_POST["scholarship"]) && !empty($_POST["abody"])) 
    {
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
         ...
         $stmt = "UPDATE researchaw SET year='$year', scholarship='$scholarship',
          abody= '$abody' WHERE researchaw_id='$val'";
        ...
        <div class="row">
            ...
            <main role="main" class="ml-sm-12 col-12">
                <div class="tm-section-wrap">
                    ...
                    <h3 class="head2"> Edit To Research Awards</h3>
                    <br>
                    <br>
                    <?php
                       require_once "pdo.php";
                       $stmt = $pdo->prepare("SELECT * FROM researchaw where researchaw_id = :val");
                       $stmt->execute(array(":val" => $_GET['value']));
                       $row = $stmt->fetch(PDO::FETCH_ASSOC);
                         $y = htmlentities($row['year']);
                         $s = htmlentities($row['scholarship']);
                         $a = htmlentities($row['abody']);
                         $val = $row['researchaw_id']; 
                        ...
                        <form action="" method="POST" class="form">
                            <div class="row">
                               <div class="col-25">
                                ...
                                <div class="col-65">
                                   <input type="text" name="scholarship" value="<?= $s ?>">
                                </div>
                                <br>
                                <div class="col-25">
                                ...
                                <div class="col-65">
                                   <input type="text" name="abody" value="<?= $a ?>">
                                </div>
                                ...
                                <a href="researchaw_action.php" class="cancelbtn"
