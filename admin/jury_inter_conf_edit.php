    ...
    if (
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
         ...
         $stmt = "UPDATE jury_inter_conf SET title='$title', conf_name='$conf',
          date= '$date' WHERE jury_inter_conf_id='$val'";
           
         
         $conn->exec($stmt);
         
         echo "<script>window.alert('Status Changed to Available');</script>";
         echo "<script>window.location.assign('jury_inter_conf_action.php');</script>";
        ...
        <div class="row">
            ...
            <main role="main" class="ml-sm-12 col-12">
                <div class="tm-section-wrap">
                    <h2 class="heading">Jury / Moderator /  </h2>
                    <h2 class="heading2"> Panelist / Resource Person </h2>
                    <h3 class="head2"> Edit To International Conference</h3>
                    <br>
                    <br>
                    <?php
                       require_once "pdo.php";
                       $stmt = $pdo->prepare("SELECT * FROM jury_inter_conf where jury_inter_conf_id = :val");
                       $stmt->execute(array(":val" => $_GET['value']));
                       $row = $stmt->fetch(PDO::FETCH_ASSOC);
                         $t = htmlentities($row['title']);
                         $c = htmlentities($row['conf_name']);
                         $d = htmlentities($row['date']);
                         $val = $row['jury_inter_conf_id']; 
                    ...
                    <div class="container-insert">
                        <form action="" method="POST" class="form">
                            <div class="row">
                                ...
                                <a href="jury_inter_conf_action.php" class="cancelbtn">Cancel</a>
                                </div>
                            </div>
                        </form>
                    </div>
