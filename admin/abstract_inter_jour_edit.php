...
if (empty($_SESSION['email'])) {
  echo "<script>window.location.assign('login.php');</script>";
    ...
    if (
        isset($_POST["submit"]) && !empty($_POST["author_name"])
        && !empty($_POST["title"]) && !empty($_POST["pub"])
        && !empty($_POST["page"])
    ) {
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
         ...
         $stmt = "UPDATE abstract_int_journal SET title='$title', author_name='$author_name',
          publication ='$pub', page ='$page' WHERE abstract_int_journal_id='$val'";
           
         
         $conn->exec($stmt);
         
         echo "<script>window.alert('Status Changed to Available');</script>";
         echo "<script>window.location.assign('abstract_inter_jour_action.php');</script>";
        ...
        <div class="row">
            ...
            <main role="main" class="ml-sm-12 col-12">
                <div class="tm-section-wrap">
                    ...
                    <h3 class="head2"> Edit To International Journal</h3>
                    <br>
                    <br>
                    <?php
                       require_once "pdo.php";
                       $stmt = $pdo->prepare("SELECT * FROM abstract_int_journal where abstract_int_journal_id = :val");
                       $stmt->execute(array(":val" => $_GET['value']));
                       $row = $stmt->fetch(PDO::FETCH_ASSOC);
                         $t = htmlentities($row['title']);
                         $a = htmlentities($row['author_name']);
                         $p = htmlentities($row['publication']);
                         $pa = htmlentities($row['page']);
                         $val = $row['abstract_int_journal_id']; 
                    ...
                    <div class="container-insert">
                        <form action="" method="POST" class="form">
                            <div class="row">
                                ...
                                <br>
                                <div class="col-25">
                                   <label for="page"> Abstract Volumn </label>
                                </div>
                                <div class="col-65">
                                ...
                                <a href="abstract_inter_jour_action.php" class="cancelbtn">Cancel</a>
                                </div>
                            </div>
                        </form>
                    </div>
