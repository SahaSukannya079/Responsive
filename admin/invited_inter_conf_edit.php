    ...
    if (
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
         ...
         $stmt = "UPDATE inter_conf SET title='$title', conf_name='$conf',
          date= '$date' WHERE inter_conf_id='$val'";
           
         
         $conn->exec($stmt);
         
         echo "<script>window.alert('Status Changed to Available');</script>";
         echo "<script>window.location.assign('invited_inter_conf_action.php');</script>";
...
<!--<li class="nav-item">
            ...
            <main role="main" class="ml-sm-12 col-12">
                <div class="tm-section-wrap">
                    ...
                    <h3 class="head2"> Edit To International Conference</h3>
                    <br>
                    <br>
                    <?php
                       require_once "pdo.php";
                       $stmt = $pdo->prepare("SELECT * FROM inter_conf where inter_conf_id = :val");
                       $stmt->execute(array(":val" => $_GET['value']));
                       $row = $stmt->fetch(PDO::FETCH_ASSOC);
                         $t = htmlentities($row['title']);
                         $c = htmlentities($row['conf_name']);
                         $d = htmlentities($row['date']);
                         $val = $row['inter_conf_id']; 
                    ...
                    <div class="container-insert">
                        <form action="" method="POST" class="form">
                            <div class="row">
                                ...
                                <div class="col-65">
                                    <input type="text" name="title" list="editors" placeholder="Enter title"
                                    value="<?= $t ?>">
                                ...
                                <div class="col-65">
                                    <input type="text" name="conf" list="con" placeholder="Enter Conference Name & Place"
                                    value="<?= $c ?>"> 
                                    <datalist id="con" required>
                                      <option value="N/A">N/A
                                    </datalist>
                                </div>
                                <br>
                                ...
                                <a href="invited_inter_conf_action.php" class="cancelbtn">Cancel</a>
                                </div>
                            </div>
                        </form>
                    </div>
