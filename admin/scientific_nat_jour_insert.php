...
if (empty($_SESSION['email'])) {
  ...
  if (
    ...
    try {
      ...
      $stmt = "insert into scientific_nat_journal
      (scientific_nat_journal_id,admin_id,title,author_name,publication,page,PMID,DOI) 
            values(' ','$admin_id','$title', '$author_name', '$pub', '$page','$pmid', '$doi');";

      $conn->exec($stmt);

      echo "<script>window.alert('scientific national journal Posted to the CMS System Successful!!');</script>";
      echo "<script>window.location.assign('scientific_nat_jour_action.php');</script>";
      
      header("Location:scientific_nat_jour_action.php");
